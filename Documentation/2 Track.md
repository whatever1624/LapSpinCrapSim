...
| --- | --- | --- |
| `xyzCoords` | 2D NumPy array of floats | Coordinate array where each index is an [x, y, z] coordinate, and each subsequent index increases the distance along the track. |
...
Makes sure that the attribute `xyzCoords` is closed if `BClosedTrack == True` (i.e. last coordinate == first coordinate), then calculates the `sCoords` and `AHeadings` arrays
...
| `BAllowNegativeInitAHeading` | Boolean | If true, will wrap the initial heading angle to the range from -pi to pi
...
This should be set to `True` if the initial direction of the track is close to 0 (or 2 pi radians) - intended to keep `AHeadings` consistent between different `CoordinateArray` objects |

## `getReducedCoordArray()`

Returns a `CoordinateArray` object with the arrays of attributes “reduced” to only those within the specified window defined by cumulative distance and heading angle bounds

Values on the bounds of the window are linearly interpolated

If the track is not closed, and the distance and heading angle bounds dictate that the reduced coordinate array should extend beyond the defined coordinates, linearly extrapolates from the first/last low-pass filtered heading angle in `AHeadings`
...
| *Name* | *Type* | *Description* |
| --- | --- | --- |
| `sRef` | Float | Reference cumulative distance, from which to get the reduced coordinate array around |
| `sLower` | Float | Lower bound for the cumulative distance along the coordinate array (i.e. lower bound of `sCoords`) |
| `sUpper` | Float | Upper bound for the cumulative distance along the coordinate array (i.e. upper bound of `sCoords`) |
| `ALower` | Float | Lower bound for the low-pass filtered cumulative heading angle (i.e. lower bound of `AHeadingsFilt`) |
| `AUpper` | Float | Upper bound for the low-pass filtered cumulative heading angle (i.e. upper bound of `AHeadingsFilt`) |
| `BClosedTrack` | Boolean | If true, the coordinate array will be treated as being closed
...
Dataclass which contains information about the event gate
...
Generates the gate line from the midpoint coordinate and heading angle
...
| --- | --- | --- |
| `xyMidpoint` | NumPy array of floats | Midpoint of the gate between the soft track limits, only on the 2D plane [x, y] |
| `AHeading` | Float or None | Heading angle of the gate, with 0 corresponding to positive y (north) and increasing clockwise

This should be the unwrapped heading in the same rotation corresponding to the `CoordinateArray` objects |
| `lLeft`

*Optional, defaults to `GATE_MAX_WIDTH / 2` | Float | Width of the gate to the left of the midpoint |
| `lRight`

*Optional, defaults to `GATE_MAX_WIDTH / 2` | Float | Width of the gate to the right of the midpoint |
| `event`

*Optional, defaults to `None` | Event object or None | If this is an `Event` object, then this gate is an event gate (and information about the event is in the `Event` object)
...
If no intersection was found, returns `(GATE_MAX_WIDTH / 2, 0)`

**Arguments**

| *Name* | *Type* | *Description* |
| --- | --- | --- |
| `reducedCoordArray` | `CoordinateArray` object | Reduced `CoordinateArray` object for the local region around the gate |
...
If None, uses `lLimitLeftHard + GATE_EXTEND_WIDTH` |
| `lRight`

*Optional, default to `None` | Float | New right width of the gate

If None, uses `lLimitRightHard + GATE_EXTEND_WIDTH` |
...
Raises an exception if `lLimitLeftSoft` and/or `lLimitRightSoft` are None
...
| `mesh` | NumPy array of SciPy interpolators | Interpolators for the local track z coordinate around each gate |

## `__init__()`

Initialises a `Track` object, either by loading the `TRACK_PKL_FILENAME` file in the provided folder, or if that file does not exist/fails to load, by generating the track from the files in the provided folder

See the functions `initFromPkl()` and `initFromTrackGen()` below

**Arguments**

| *Name* | *Type* | *Description* |
| --- | --- | --- |
| `trackPath` | String | Path to the folder containing the required files to load the Track object from the `TRACK_PKL_FILENAME` file, or to generate the track if the track pkl file does not exist |
...
If this is None, automatically determines whether the track is closed or not by whether the maximum distance from the start to finish points of the track limit arrays is less than `CLOSED_TRACK_THRESHOLD_DISTANCE` |
| `BForceTrackGen`
...
If false, the `TRACK_PKL_FILENAME` file will try to be loaded, and only if it doesn’t exist/fails to load will the track be generated |

## `__initFromPkl()`

Internal function to initialise the `Track` object by loading the pkl file at path pklPath

**Arguments**

| *Name* | *Type* | *Description* |
| --- | --- | --- |
| pklPath | String | Path to the track pkl file |

## `__initFromTrackGen()`

Internal function to initialise the Track object by generating it from the data in the `trackPath` folder, using the logic detailed below
...
*Can have multiple files beginning with ‘eventData’ and ending with ‘.json’ to allow multiple events | Contains the keys listed below, which are used to specify an event
...
`xyStartLeft`: Left coordinate of the event start gate, as a list of floats in the form [xStartLeft, yStartLeft]

`xyStartRight`: Right coordinate of the event start gate, as a list of floats in the form [xStartRight, yStartRight]

`xyFinishLeft`: Left coordinate of the event finish gate, as a list of floats in the form [xFinishLeft, yFinishLeft]

`xyFinishRight`: Right coordinate of the event finish gate, as a list of floats in the form [xFinishLeft, yFinishLeft]

`properties` (*Optional): Dictionary of properties for the event, only needed if the event requires properties to define it

The name used for the event is parsed from the filename (i.e. the unique filename substring represented by the ** )
...
These are expected to be within the hard track limits (i.e. enclosed by `limitLeftHard` and `limitRightHard`), though some tolerance is permitted by the gate extending some distance beyond the hard track limits |

### `__initGateFromCoords()`

Initialise a `Gate` object with the minimum initialisation by processing the left and right coordinates into the midpoint coordinate and heading angle before passing them into the `Gate` object initialisation function
...
Internal function which follows the fallback logic detailed above in the expected file structure of `trackPath`, to parse the track files and return the tuple of `(coordArraysDict, eventGatesDict)`:
...
- `eventGatesDict`
    ...
    - For the ‘StartFinish’ key, its value must be the a list containing 2 `Gate` objects, in the order such that the start gate is index 0 and the finish gate is index 1
...
| *Name* | *Type* | *Description* |
| --- | --- | --- |
| `trackPath` | String | Path to the folder containing the required files to generate the track |
...
| *Name* | *Type* | *Description* |
| --- | --- | --- |
| `trackPath` | String | Path to the folder to save the track pkl file |
| `coordArraysDict` | Dictionary | Dictionary containing the `CoordinateArray` objects for track limits and any extra coordinate arrays provided |
...
Sets `axis(’equal’)` to display in the correct aspect ratio
...
- Coloured by event type, with the special cases of:
    ...
    - Green if start gate
    - Red if finish gate
- Dashed line if the gate index is in `badGateInds`

Scatter plot track gate intersections with the track limits (group by event type and track limit type, then use `plot()` with the kwarg `linestyle=''` to be faster)
...
Gets `coordArraysDict` and `eventGatesDict` using `__parseTrackFiles()`
...
- Check if this gate intersects with the gate with the event that has both attributes `type == 'StartFinish'` and `BStart == True` (i.e. the start gate) - if it intersects, uses this event gate for the first gate instead and repeat the point above for the this new first gate
...
- If the track is closed, copy the first gate as the final gate, then modify the gate width to only span the width of the soft track limits and modify the event attribute `BStart` to false
...
Add the ‘GateCreation’ key to `eventGatesDict`, with the value being the list containing only the final gate

**Gate Creation Loop**

Run the loop detailed below until the gate creation has reached the last gate, denoted by the flag `BStopGateCreation` being true

This loop iterates around the track creating gates in regular intervals, making sure to include event gates in the correct order, and stops once it has gone around the whole track

1. Get the reduced track limit `CoordinateArray` object for each of the track limits (”reduced” as they are for the local region around the gate) - as `reducedLimitLeftSoft`, `reducedLimitRightSoft`, `reducedLimitLeftHard`, and `reducedLimitRightHard`
    1. Uses the function `getReducedCoordArray()`, with the arguments:
        1. `sLower = 0`
        2. `sUpper = REDUCED_DISTANCE_WINDOW`
        3. `ALower = -REDUCED_HEADING_WINDOW`
        4. `AUpper = REDUCED_HEADING_WINDOW`
2. Gets the Shapely LineStrings created from the coordinates of each of the reduced track limit `CoordinateArray` objects - as `reducedLimitLeftSoftLS`, `reducedLimitRightSoftLS`, `reducedLimitLeftHardLS`, and `reducedLimitRightHardLS`
3. Optimise the new gate placement with SciPy optimise
    1. Find the 2 parameters `[psi, theta]` to place the gate midpoint in the centre of the track, facing the direction of travel, with an initial guess of `[0, 0]`
        1. The midpoint of the new gate will be at distance `GATE_STEP_DISTANCE` from the midpoint of the previous gate
        2. `psi` is the anti-clockwise angle in radians from the previous gate heading to place the midpoint of the new gate
        3. `theta` is the anti-clockwise heading angle offset in radians of the new gate, relative to `psi`
    ...
    4. Raise an exception if the gate creation exploded (optimised parameters cause the gate midpoint to go beyond the bounding box `[xMin, xMax, yMin, yMax]` of the hard track limits - but run `__saveTrackPlot()` first with this bad gate added to the `gates` list, and the `badGateInds` list passed for debugging
4. Create the new gate from the optimised parameters
...
5. Create the flag `BValidGate` for whether this new gate is valid (and whether it should be added to the `gates` list) or invalid (intersects with the previous or next gate therefore should not be added to the `gates` list)
    1. Initialise this flag to `True`
6. Create a list of event gates contained by the track segment from the previous gate to the new gate, sorted by (ascending) distance of their midpoints from the midpoint of the previous gate
...
7. If the list of event gates contained by the track segment from the previous gate to the new gate is not empty:
    ...
    2. Check if consecutive event gates intersect with each other at a single point (multiple event gates can share the same line)
        1. If an intersection is detected, raise an exception (possible fixes are: redefine the event gates to be more spaced out, reduce `GATE_EXTEND_WIDTH`, increase `DIRECTION_SIMILARITY_THRESHOLD`) - but run `__saveTrackPlot()` first with this bad gate added to the `gates` list, and the `badGateInds` list passed for debugging
...
8. If `BValidGate` is still true, check if the new gate intersects with the previous gate by checking the gate lines with the Shapely `intersects()` function - If they intersect, set the flag `BValidGate` to `False`
9. If `BValidGate` is still true, append the new gate to the `gates` list

**Postprocessing and Validation**

Run `__saveTrackPlot()`

Convert the `gates` list to a NumPy array
...
Create an empty NumPy array `mesh` of the same length of the gates array, which will contain SciPy multivariate interpolator objects (one of the interpolators for unstructured data) - this will be the meshArray representing the track mesh
...
Pickle the `Track` object using `pkl.dump(self)` to the `TRACK_PKL_FILENAME` file in `trackPath`
...
| `gateInd` | Integer | Index of the gate, which determines which local track mesh interpolator to use to calculate the z coordinate |
| `BReturnNaN`
...
| `gateInd` | Integer | Index of the gate, which determines which local track mesh interpolator to use to calculate the track normal |

### Calculation Logic

If the z coordinate is not provided, calculates it using `calcTrackZ()` with the argument `BReturnNaN = True` - if the z coordinate is NaN, returns the failed track normal `[0, 0, 1]`

In each of the x and y directions:

- Perturbs the specified coordinate in the positive direction, then uses `calcTrackZ()` with the argument `BReturnNaN = True` to calculate the track z coordinate at the perturbed location
...
Calculates the track normal as the cross product between the (unit) vectors from the perturbed points (in the order Y cross X such that the calculated normal points in positive z)